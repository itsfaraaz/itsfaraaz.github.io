...
<ol>
  <li>Get the author email with <code class="language-plaintext highlighter-rouge">npm owner ls &lt;pkgname&gt;</code></li>
...
<ol>
  <li>Alice writes a JavaScript module <code class="language-plaintext highlighter-rouge">foo</code>, which is not node-specific. Alice
doesn’t use node at all. Yusuf wants to use <code class="language-plaintext highlighter-rouge">foo</code> in node, so he wraps it in
an npm module. Some time later, Alice starts using node, and wants to take
over management of her program.</li>
  <li>Yusuf writes an npm module <code class="language-plaintext highlighter-rouge">foo</code>, and publishes it. Perhaps much later, Alice
finds a bug in <code class="language-plaintext highlighter-rouge">foo</code>, and fixes it. She sends a pull request to Yusuf, but
...
with node any more. Alice would like to publish a new <code class="language-plaintext highlighter-rouge">foo</code>, but can’t,
because the name is taken.</li>
  <li>Yusuf writes a 10-line flow-control library, and calls it <code class="language-plaintext highlighter-rouge">foo</code>, and
...
critically acclaimed and widely-marketed <code class="language-plaintext highlighter-rouge">foo</code> JavaScript toolkit framework.
They publish it to npm as <code class="language-plaintext highlighter-rouge">foojs</code>, but people are routinely confused when
<code class="language-plaintext highlighter-rouge">npm install foo</code> is some different thing.</li>
  <li>
    <p>Yusuf writes a parser for the widely-known <code class="language-plaintext highlighter-rouge">foo</code> file format, because he
needs it for work. Then, he gets a new job, and never updates the prototype.
Later on, Alice writes a much more complete <code class="language-plaintext highlighter-rouge">foo</code> parser, but can’t publish,
because Yusuf’s <code class="language-plaintext highlighter-rouge">foo</code> is in the way.</p>
  </li>
  <li><code class="language-plaintext highlighter-rouge">npm owner ls foo</code>. This will tell Alice the email address of the owner
(Yusuf).</li>
...
that Yusuf can run npm owner <code class="language-plaintext highlighter-rouge">add alice foo</code> to add Alice as an owner of the
foo package.</li>
...
clear up any confusion with changes to their package’s <code class="language-plaintext highlighter-rouge">README</code> file or
metadata.</p>
